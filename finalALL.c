...
void onDNSClicked(GtkWidget* widget)
{
    ...
    if(dnsfp==0)
        return ;
    char output[10000];
    while(fgets (output, 10000, dnsfp))
    {
       if(strcmp(output,"**********************************************************\n")==0)
           break;
        gtk_text_buffer_insert(buff,&tvstart,output,-1);
    ...
    gtk_text_buffer_get_end_iter(buff,&tvstart);
    currDNS++;
    gtk_text_buffer_get_start_iter(buffDNS,&tvstart);
    char no[10];
    sprintf(no, "%d", currDNS);
    gtk_text_buffer_insert(buffDNS,&tvstart,no,-1);
    gtk_text_buffer_get_end_iter(buffDNS,&tvend);
    gtk_text_buffer_delete(buffDNS,&tvstart,&tvend);
    gtk_text_buffer_get_end_iter(buffDNS,&tvstart);
